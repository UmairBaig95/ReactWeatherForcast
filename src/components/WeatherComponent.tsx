...
class WeatherComponent extends React.Component {
  componentDidMount(): void { 
    axios.get(`https://api.openweathermap.org/data/2.5/forecast?lat=${this.state.selectedLocation.lat}&lon=${this.state.selectedLocation.lon}&appid=d31f3093c99470ccd7b38f3e28bbe8bb&&units=metric`).then(res => {
        let dateList:any[] = [];
        let itemList:any[] = []
    ...
    return (
      <div className="container center">
        ...
        <div className="card weatherContent">
         {this.state.locationResult.length > 0 && <TodayCard  {...this.state.locationResult}  /> } 
         {this.state.locationResult.length > 0 && <UpcomingCard {...this.state.locationResult} />}
        </div>
      </div>
    );
  }
